	...
	describe('CLI', ({ describe, test }) => {
		describe('version', ({ test }) => {
			test('shows version', async () => {
				...
				expect(tunProcess.stdout).toBe(
					`tun v${packageJson.version}\nnode ${process.version}`
				);
			...
			test("doesn't show version with file", async () => {
				const tunProcess = await tun({
					args: [path.join(fixturePath, 'log-argv.ts'), '--version']
				});
		...
		describe('help', ({ test }) => {
			...
			test("doesn't show help with file", async () => {
				const tunProcess = await tun({
					args: [path.join(fixturePath, 'log-argv.ts'), '--help']
				});
		...
		test('Node.js test runner', async () => {
			const tunProcess = await tun({
				args: ['--test', path.join(fixturePath, 'test-runner-file.ts')]
			});
				...
				test(
					...
					async () => {
						const tunProcess = tun({
							args: [path.join(fixturePath, 'catch-signals.js')]
						});
						...
						if (process.platform === 'win32') {
							...
							expect(tunProcessResolved.stdout).toBe(
