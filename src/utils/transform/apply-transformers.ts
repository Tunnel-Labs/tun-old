...
type IntersectionArray<T extends unknown[]> = T extends [
	infer FirstElement,
	...infer RestElements
]
...
type Transformer<ReturnType extends MaybePromise<TransformerResult>> = (
	filePath: string,
	code: string
) => ReturnType;

type Results<Array_ extends Transformer<MaybePromise<TransformerResult>>[]> = {
	[Key in keyof Array_]: Array_[Key] extends Transformer<infer ReturnType>
		? Awaited<ReturnType>
		: unknown;
};
...
>(filePath: string, code: string, transformers: T) {
